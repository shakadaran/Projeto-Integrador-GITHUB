...
public class CastSpell : MonoBehaviour {
	...
	/*	if(Input.GetMouseButtonDown(0) && !castingSpell)
        ...
        */
		if(Input.GetKey("v") && !castingSpell)
		{
	...
	void SpellSetUp(Spell spell)
	{
		...
		if(spell.spellType == Spell.SpellType.Single)
		{
			...
			{
				spellObject = (GameObject)Instantiate(spell.spellPrefab,myTransform.position + myTransform.up+transform.forward*2,myTransform.rotation);
	...
	public IEnumerator SpellCast(Spell spell)
	{
		...
		if(spell.spellManaCost <= mana){
            ...
            //Wait for choosen spell cast time.
            myTransform.GetComponent<Animator>().SetBool("Casting", true);
        ...
        myTransform.GetComponent<Animator>().SetBool("Casting", false);
        //Wait 1 second.
        yield break;
		//end.
	}
